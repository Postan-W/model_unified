...
def get_dims():
    ...
    partten1 = '$.metadata.signature_def.signature_def.test_signature.inputs..dim[*].size'
    partten2 = '$.metadata.signature_def.signature_def.prediction_signature.inputs..dim[*].size'

    dims_list = jsonpath.jsonpath(json_obj, partten1)
    if dims_list == False:
        dims_list = jsonpath.jsonpath(json_obj, partten2)
    ...
    def post(self):
        ...
        if type == 'b64':
            ...
            try:
                ...
                predict_request = json.dumps(
                    {"signature_name": signature_name, "instances": instances})  # 字符串格式没问题，但和python base64算法标准不一样
            except Exception as e:
        ...
        # url方式
            ...
            try:
                ...
                predict_request = json.dumps({"signature_name": signature_name, "instances": instances})
            except Exception as e:
                logging.error({'error': str(e)})
                self.write({'error': str(e)})
                return
        ...
        # data数组方式
            ...
            try:
                ...
                if not dims_list:
                    dims_list = get_dims()
                predict_request = json.dumps({"signature_name": signature_name, "instances": data})
            except Exception as e:
                logging.error(e)
                self.write({'error': str(e)})
                return
