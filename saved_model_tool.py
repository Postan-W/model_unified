...
def show_all(saved_model_dir):
    ...
    for tag_set in sorted(tag_sets):
        ...
        for signature_def_key in sorted(signature_def_map.keys()):
            ...
            # 第一版后期整合出现问题
            # if not inputs_tensor_info:
            #     continue
            # if not outputs_tensor_info:
            #     continue
            #
            # inputs_info = {}
            # outputs_info = {}
            ...
            #
            #
            # for k,v in inputs_tensor_info.items():
            #     inputs_info[k] = str(v)
            #     inputs.append(k)
            #     inputs_shape[k] = [int(i) for i in re.findall("size: (.\d+)", str(v))]
            #
            #
            # for k,v in outputs_tensor_info.items():
            #     outputs_info[k] = str(v)
            #     outputs.append(k)
            #
            # return inputs_info,outputs_info,inputs,inputs_shape,outputs

            # 现在改为新版本
            result[signature_def_key] = {"inputs": str(inputs_tensor_info), "outputs": str(outputs_tensor_info)}
    return result
